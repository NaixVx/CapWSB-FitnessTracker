...
class DatabaseSchemaTest {
    ...
    void trainingsTableHasExpectedColumns() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            assertThat(cols).contains("id", "user_id", "activity_type", "start_time", "end_time", "average_speed",
                    "distance");
        }
    }
    ...
    void workoutSessionTableHasExpectedColumns() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            assertThat(cols).contains("id", "training_id", "altitude", "end_latitude", "end_longitude",
                    "start_latitude", "start_longitude", "timestamp");
        }
    }
